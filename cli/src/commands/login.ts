...
/**
 * Stub. ape-pr does not manages its own login flow — auth is shared
...
export const loginCommand = defineCommand({
  ...
  async run() {
    info('ape-pr uses the unified `apes` auth session.')
    info('')
    info('Run `apes login <email>` once on this device, then `ape-pr …` works')
    ...
    info('')
    error('No-op: ape-pr login is a stub.')
    process.exit(1)
  },
})
